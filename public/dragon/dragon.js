var sketchProc = function ( processingInstance ) {
  with ( processingInstance )
  {
    ...
    // Función para crear partículas CSS
    ...
    // Función para cambiar tema visual en CSS
    function applyThemeToCSS(theme) {
      const body = document.body;
      const themes = ['red', 'green', 'blue', 'purple', 'gold', 'black'];
      
      // Remover todas las clases de tema
      themes.forEach(t => body.classList.remove(`theme-${t}`));
      
      // Agregar la nueva clase de tema
      body.classList.add(`theme-${themes[theme]}`);
      
      // Agregar efecto de resplandor ocasionalmente
      if(Math.random() > 0.7) {
        canvasElement.classList.add('canvas-glow');
        setTimeout(() => {
          canvasElement.classList.remove('canvas-glow');
        }, 4000);
      }
    }
    ...
    var Dragon = function () {
      ...
      this.colorThemes = {
        ...
        blue: {
          spikes: color( 15, 35, 71 ),
          black: color( 45, 55, 85 ),
          body: color( 25, 45, 128 ),
          wingArm: color( 20, 35, 107 ),
          wingArmSpike: color( 76, 77, 76 ),
          wingUpper: color( 30, 50, 122 ),
          wingLower: color( 40, 65, 148 ),
          wingSpines: color( 71, 70, 71, 150 ),
          legsBehind: color( 22, 40, 110 )
        },
        purple: {
          spikes: color( 51, 14, 71 ),
          black: color( 75, 45, 85 ),
          body: color( 88, 25, 128 ),
          wingArm: color( 67, 20, 107 ),
          wingArmSpike: color( 76, 77, 76 ),
          wingUpper: color( 82, 30, 122 ),
          wingLower: color( 98, 40, 148 ),
          wingSpines: color( 71, 70, 71, 150 ),
          legsBehind: color( 70, 22, 110 )
        },
        gold: {
          spikes: color( 102, 77, 15 ),
          black: color( 120, 100, 30 ),
          body: color( 184, 134, 11 ),
          wingArm: color( 153, 111, 8 ),
          wingArmSpike: color( 76, 77, 76 ),
          wingUpper: color( 212, 155, 31 ),
          wingLower: color( 255, 193, 37 ),
          wingSpines: color( 71, 70, 71, 150 ),
          legsBehind: color( 163, 119, 10 )
        }
      };
      ...
      this.draw = function () {
        pushMatrix();
        // Mover el dragón más hacia la derecha (aumentar el valor para más hacia la derecha)
    ...
    var Scene = function () {
      this.page = "load";
      this.loaded = false;
      this.imageIndex = 0; // Agregar esta propiedad que faltaba
      this.dragon = new Dragon();
      this.cloudsBack = [];
      this.cloudsFront = [];
      this.showClouds = true; // Agregar esta propiedad que faltaba
      this.grounds = [
        {
          x: 0
        },
      ...
      this.setup = function () {
        this.images = {
          ...
          backPurple: function () {
            ...
            for ( var i = 0; i <= 450; i++ )
            {
              fill( lerpColor( color( 52, 20, 82 ), color( 93, 56, 148 ), i / 500 ) );
              rect( 0, i, width, i );
            }
          ...
          backGold: function () {
            ...
            for ( var i = 0; i <= 450; i++ )
            {
              fill( lerpColor( color( 102, 77, 52 ), color( 161, 121, 82 ), i / 500 ) );
              rect( 0, i, width, i );
            }

            return get( 0, 0, width, 450 );
          },
          backDeepBlue: function () {
            background( 0, 0 );

            noStroke();
            for ( var i = 0; i <= 450; i++ )
            {
              fill( lerpColor( color( 15, 25, 65 ), color( 35, 55, 95 ), i / 500 ) );
              rect( 0, i, width, i );
            }

            return get( 0, 0, width, 450 );
          },
          groundPurple: function () {
            noStroke();

            for ( var i = 0; i <= 150; i++ )
            {
              fill( lerpColor( color( 47, 35, 63 ), color( 67, 47, 92 ), i / 150 ) );
              rect( 0, 450 + i, width, 450 + i );
            }

            for ( var i = 0; i < 200; i++ )
            {
              fill( random( 40, 60 ), random( 25, 40 ), random( 50, 70 ), random( 50, 150 ) );
              var radius = random( 2, 5 );
              ellipse( random( width ), random( 470, 600 ), radius, radius * 0.5 );
            }
          ...
          groundGold: function () {
            ...
            for ( var i = 0; i <= 150; i++ )
            {
              fill( lerpColor( color( 77, 63, 38 ), color( 102, 87, 54 ), i / 150 ) );
              rect( 0, 450 + i, width, 450 + i );
            }

            for ( var i = 0; i < 200; i++ )
            {
              fill( random( 80, 100 ), random( 60, 80 ), random( 20, 40 ), random( 50, 150 ) );
              var radius = random( 2, 5 );
              ellipse( random( width ), random( 470, 600 ), radius, radius * 0.5 );
            }
          ...
          groundBlue: function () {
            ...
            for ( var i = 0; i <= 150; i++ )
            {
              fill( lerpColor( color( 35, 47, 79 ), color( 54, 67, 102 ), i / 150 ) );
              rect( 0, 450 + i, width, 450 + i );
            }

            for ( var i = 0; i < 200; i++ )
            {
              fill( random( 30, 50 ), random( 40, 60 ), random( 60, 80 ), random( 50, 150 ) );
              var radius = random( 2, 5 );
              ellipse( random( width ), random( 470, 600 ), radius, radius * 0.5 );
            }
      ...
      this.load = function ( s ) {
        ...
        // Verificar que no hayamos terminado de cargar
        ...
        // Verificar si hemos terminado de cargar
        if ( this.imageIndex >= obj.length ) {
          ...
          // Ocultar loading con animación CSS
          if (loadingContainer) {
      ...
      // Agregar método draw que faltaba
      this.draw = function() {
        ...
        // Dibujar las colinas traseras
        if (this.images && this.images.hillsBack) {
          for (var i = 0; i < this.hillsBack.length; i++) {
            pushMatrix();
            translate(this.hillsBack[i].x, 250);
            image(this.images.hillsBack, 0, 0);
            popMatrix();

            this.hillsBack[i].x += 1;
            if (this.hillsBack[i].x > width) {
              this.hillsBack[i].x = -599;
            }
          }
        }

        // Dibujar las colinas frontales
        if (this.images && this.images.hillsFront) {
          for (var i = 0; i < this.hillsFront.length; i++) {
            pushMatrix();
            translate(this.hillsFront[i].x, 350);
            image(this.images.hillsFront, 0, 0);
            popMatrix();

            this.hillsFront[i].x += 2;
            if (this.hillsFront[i].x > width) {
              this.hillsFront[i].x = -599;
            }
          }
        }
        ...
        if (this.groundImage) {
          for (var i = 0; i < this.grounds.length; i++) {
            pushMatrix();
            translate(this.grounds[i].x, 450);
            image(this.groundImage, 0, 0);
            popMatrix();
      ...
      // Agregar método runClouds que faltaba
      ...
      this.setTheme = function () {
        this.theme = ( this.theme + 1 ) % this.totalThemes;
        
        // Aplicar tema también al CSS
        ...
        switch ( this.theme ) {
          ...
          case 2:
            ...
            break;
          case 3:
            this.skyImage = this.images.backDeepBlue;
            this.groundImage = this.images.groundBlue;
            this.dragon.colors = this.dragon.colorThemes.blue;
            break;
          case 4:
            this.skyImage = this.images.backPurple;
            this.groundImage = this.images.groundPurple;
            this.dragon.colors = this.dragon.colorThemes.purple;
            break;
          case 5:
            this.skyImage = this.images.backGold;
            this.groundImage = this.images.groundGold;
            this.dragon.colors = this.dragon.colorThemes.gold;
            break;
        }
      };
      ...
      this.go = function () {
        ...
        // Cambio automático de tema cada 3 segundos
        this.themeTimer++;
        if ( this.themeTimer >= this.themeChangeInterval )
        {
          this.setTheme();
          this.themeTimer = 0;
        }
    ...
    // Manejo responsivo del canvas
    function handleResize() {
      const canvas = document.getElementById('canvas');
      if(canvas && window.innerWidth < 768) {
        size(window.innerWidth * 0.95, 400);
      } else {
        size(window.innerWidth, 600);
      }
    }
